...
$(document).ready(function() {
    ...
    var isSearchActive2 = false; // 검색 상태를 추적하는 변수
        function fetchCategories1(page = 1, size = 10) {
            ...
            if(isSearchActive1){
                ...
                $.ajax({
                    url: `/api/store/list?&name=${name}&size=${size}&search=`,
                    method: 'GET',
    ...
    function fetchCategories2(page = 1, size = 10) {
        ...
        if(isSearchActive2){
            ...
            $.ajax({
                url: `/api/store/list?&parentId=${parentId}&size=${size}&search=`,
                method: 'GET',
                dataType: 'json', // 응답 데이터 타입
                data:{
                    page: page
                ...
                success: function(data) {
                    ...
                    function loadNewCategories(page) {
                        $.ajax({
                            url: `/api/store/list2?&parentId=${parentId}&size=${size}&search=`,
                            type: 'GET',
                            dataType: 'json',
                            data: {
                                page: page
                    ...
                    function loadNewPage(page) {
                        $.ajax({
                            url: `/api/store/list2?&name=${name}&size=${size}&search=`,
                            type: 'GET',
                            data: {
                                page: page
                            },
    ...
    $('#deleteConfirmModal .btn-danger').on('click', function() {
        ...
        $.ajax({
            url: '/store/menu/category/delete/' + categoryId, // 실제 URL은 서버 API 설계에 따라 다를 수 있습니다.
